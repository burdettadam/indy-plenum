...
def testRequestStaticValidation(tconf, looper, txnPoolNodeSet,
    ...
    req = generate_invalid_unsigned_plenum_request(wallet_client, {'something': 'nothing'})
...
def test3PCOverBatchWithThresholdReqs(tconf, looper, txnPoolNodeSet,
    ...
    :return:
    """
    vdr_send_random_and_check(looper, txnPoolNodeSet, pool_handle, wallet_client, tconf.Max3PCBatchSize)
...
def test3PCOverBatchWithLessThanThresholdReqs(tconf, looper, txnPoolNodeSet,
    ...
    :return:
    """
    vdr_send_random_and_check(looper, txnPoolNodeSet, pool_handle, wallet_client, tconf.Max3PCBatchSize - 1)
...
def testTreeRootsCorrectAfterEachBatch(tconf, looper, txnPoolNodeSet,
    ...
    # Send 2 batches
    vdr_send_random_and_check(looper, txnPoolNodeSet, pool_handle, wallet_client, 2 * tconf.Max3PCBatchSize)
    checkNodesHaveSameRoots(txnPoolNodeSet)
