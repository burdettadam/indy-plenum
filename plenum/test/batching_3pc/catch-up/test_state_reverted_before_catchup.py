...
def test_unordered_state_reverted_before_catchup(
        tconf, looper, txnPoolNodeSet, wallet_client, pool_handle):
    """
    ...
    reqs = vdr_signed_random_requests(looper, wallet_client, 10)
    vdr_send_and_check(reqs, looper, txnPoolNodeSet, pool_handle)
    ...
    # send requests
    reqs = vdr_send_random_requests(looper, pool_handle, wallet_client, tconf.Max3PCBatchSize)
