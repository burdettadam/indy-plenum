...
def test_second_digest_is_written(
        looper, txnPoolNodeSet, pool_handle, vdr_wallet_stewards):
    ...
    res = vdr_get_and_check_replies(looper, vdr_send_signed_requests(pool_handle, [req], looper))
...
def test_send_same_txn_with_different_signatures_in_separate_batches(
        looper, txnPoolNodeSet, pool_handle, two_requests):
    ...
    rep1 = vdr_send_signed_requests(pool_handle, [req1], looper)
    vdr_get_and_check_replies(looper, rep1)

    rep2 = vdr_send_signed_requests(pool_handle, [req2], looper)
...
def test_send_same_txn_with_different_signatures_in_one_batch(
        looper, txnPoolNodeSet, pool_handle, two_requests, tconf):
    req1, req2 = two_requests
    ...
    with max_3pc_batch_limits(tconf, size=2):
        vdr_send_signed_requests(pool_handle, [req1], looper)
        vdr_send_signed_requests(pool_handle, [req2], looper)
...
def test_parts_of_nodes_have_same_request_with_different_signatures(
        looper, txnPoolNodeSet, pool_handle, two_requests, vdr_wallet_stewards, tconf):
    ...
    req1s = vdr_send_signed_requests(pool_handle, [req1s], looper)
    vdr_get_and_check_replies(looper, req1s)

    req2s = vdr_send_signed_requests(pool_handle, [req2s], looper)
    with pytest.raises(RequestNackedException) as e:
...
def test_suspicious_primary_send_same_request_with_same_signatures(
        looper, txnPoolNodeSet, pool_handle, two_requests, vdr_wallet_stewards, tconf):
    couple = vdr_send_random_and_check(looper, txnPoolNodeSet, pool_handle, vdr_wallet_stewards[0], 1)[0]
