...
def add_ne_node(looper, pool_handle, steward_wallet_handle):
    ...
    request_couple = vdr_sign_and_send_prepared_request(looper, steward_wallet_handle, pool_handle, node_request)
    vdr_get_and_check_replies(looper, [request_couple])
    return dest, new_node_name


def test_demote_nonexisted(looper, txnPoolNodeSet, pool_handle, tdir, tconf, vdr_wallet_new_steward):
    dst, name = add_ne_node(looper, pool_handle, vdr_wallet_new_steward)
    assert dst
