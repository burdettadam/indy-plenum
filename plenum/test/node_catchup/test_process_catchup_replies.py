...
def _add_txns_to_ledger(node, looper, wallet_client, num_txns_in_reply, reply_count):
    '''
    ...
    reqs = vdr_signed_random_requests(looper, wallet_client, txn_count)
...
def test_process_catchup_replies(txnPoolNodeSet, looper, wallet_client):
    '''
    ...
    ledger_manager.processCatchupRep(reply2, wallet_client[1])
    check_reply_not_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], reply2)

    reply3 = catchup_reps[2]
    ledger_manager.processCatchupRep(reply3, wallet_client[1])
    check_reply_not_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], reply2)
    check_reply_not_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], reply3)

    reply1 = catchup_reps[0]
    ledger_manager.processCatchupRep(reply1, wallet_client[1])
    ...
    ledger_manager.processCatchupRep(reply6, wallet_client[1])
    check_reply_not_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], reply6)

    reply4 = catchup_reps[3]
    ledger_manager.processCatchupRep(reply4, wallet_client[1])
    ledger_size = check_replies_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], [reply4])
    check_reply_not_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], reply6)

    reply5 = catchup_reps[4]
    ledger_manager.processCatchupRep(reply5, wallet_client[1])
    ledger_size = check_replies_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], [reply5,
                                                                                                         reply6])
    assert not catchup_rep_service._received_catchup_replies_from
    assert not catchup_rep_service._received_catchup_txns


def test_process_invalid_catchup_reply(txnPoolNodeSet, looper, wallet_client):
    '''
    Test correct work of method processCatchupRep and that sending replies
    in reverse order will call a few iterations of cycle in _processCatchupRep
    '''
    ...
    req = vdr_signed_random_requests(looper, wallet_client, 1)[0]
    ...
    check_reply_not_applied(ledger_size, ledger, catchup_rep_service, wallet_client[1], invalid_reply2)
    ...
    assert not catchup_rep_service._received_catchup_replies_from[wallet_client[1]]
