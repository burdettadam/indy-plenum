...
def send_aml_request(looper, wallet_trustee, pool_handle, version, aml, context):
    req = looper.loop.run_until_complete(build_acceptance_mechanisms_request(
        wallet_trustee[1],
        aml,
        version, context))
    req = vdr_sign_and_send_prepared_request(looper, wallet_trustee, pool_handle, req)
...
def test_get_taa_aml_static_validation_fails(looper, txnPoolNodeSet, pool_handle, wallet_client):
    req = {
        OPERATION: {
...
def test_get_taa_aml_works_on_clear_state(looper, txnPoolNodeSet, pool_handle, wallet_client):
    reply = sdk_get_taa_aml(looper, pool_handle, wallet_client)[1]
...
def test_get_taa_aml_can_return_taa_for_old_version(looper, nodeSetWithTaa,
                                                    pool_handle, wallet_client):
    reply = sdk_get_taa_aml(looper, pool_handle, wallet_client, version=V1)[1]
...
def test_get_taa_aml_can_return_taa_for_current_version(looper, nodeSetWithTaa,
                                                        pool_handle, wallet_client):
    reply = sdk_get_taa_aml(looper, pool_handle, wallet_client, version=V2)[1]
...
def test_get_taa_aml_doesnt_return_taa_for_nonexistent_version(looper, nodeSetWithTaa,
    ...
    reply = sdk_get_taa_aml(looper, pool_handle, wallet_client,
