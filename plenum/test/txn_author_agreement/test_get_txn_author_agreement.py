...
from plenum.test.txn_author_agreement.helper import get_txn_author_agreement, taa_digest, \
...
def nodeSetWithTaaAlwaysResponding(txnPoolNodeSet, set_txn_author_agreement_aml, looper, pool_handle,
                                   vdr_wallet_trustee):
    ...
    reply = sdk_send_txn_author_agreement(looper, pool_handle, vdr_wallet_trustee, V1, TEXT_V1, RATIFIED_V1)
    TIMESTAMP_V1 = reply[1]['result'][TXN_METADATA][TXN_METADATA_TIME]

    looper.runFor(3)  # Make sure we have long enough gap between updates
    RATIFIED_V2 = get_utc_epoch() - 30
    reply = sdk_send_txn_author_agreement(looper, pool_handle, vdr_wallet_trustee, V2, TEXT_V2, RATIFIED_V2)
    TIMESTAMP_V2 = reply[1]['result'][TXN_METADATA][TXN_METADATA_TIME]

    sdk_send_txn_author_agreement(looper, pool_handle, vdr_wallet_trustee, V1, retired=TIMESTAMP_V1)
...
def test_get_txn_author_agreement_returns_latest_taa_by_default(looper, set_txn_author_agreement_aml, nodeSetWithTaa,
    ...
    reply = get_txn_author_agreement(looper, pool_handle, wallet_client)[1]
...
def test_get_txn_author_agreement_can_return_taa_for_old_version(looper, nodeSetWithTaa,
                                                                 pool_handle, wallet_client):
    reply = get_txn_author_agreement(looper, pool_handle, wallet_client,
                                         version=V1)[1]
    assert reply['op'] == REPLY
