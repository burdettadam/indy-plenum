...
def test_send_invalid_txn_author_agreement_fails(
        looper, set_txn_author_agreement_aml, txnPoolNodeSet, pool_handle, vdr_wallet_trustee, random_taa
):
    ...
    rep = vdr_sign_and_send_prepared_request(looper, vdr_wallet_trustee, pool_handle, json.dumps(req))
    with pytest.raises(RequestNackedException):
        vdr_get_and_check_replies(looper, [rep])


def test_create_txn_author_agreement_succeeds(looper, set_txn_author_agreement_aml, pool_handle, vdr_wallet_trustee):
    # Write random TAA
    version, text, ratified = randomString(16), randomString(1024), get_utc_epoch() - 600
    sdk_send_txn_author_agreement(looper, pool_handle, vdr_wallet_trustee,
                                  version=version,
                                  text=text,
                                  ratified=ratified)
...
def test_txn_author_agreement_update_ratification_fails(looper, set_txn_author_agreement_aml,
                                                        pool_handle, vdr_wallet_trustee, ratified_offset):
    # Write random TAA
    version, text, ratified = randomString(16), randomString(1024), get_utc_epoch() - 600
    sdk_send_txn_author_agreement(looper, pool_handle, vdr_wallet_trustee,
                                  version=version,
...
def test_txn_author_agreement_retire_non_latest(looper, set_txn_author_agreement_aml,
    ...
    sdk_send_txn_author_agreement(looper, pool_handle, vdr_wallet_trustee, version=version_1,
...
def test_send_valid_txn_author_agreement_without_enough_privileges_fails(
        ...
        pool_handle, vdr_wallet_steward, random_taa
):
    with pytest.raises(RequestRejectedException):
        text, version = random_taa
        sdk_send_txn_author_agreement(looper, pool_handle, vdr_wallet_steward, version, text)
