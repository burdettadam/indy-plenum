...
def test_old_txn_metadata_digest_fallback(looper, wallet_client):
    # Create signed request and convert to legacy txn
    req_str = vdr_signed_random_requests(looper, wallet_client, 1)[0]
...
def test_old_txn_metadata_multisig_digest_fallback(looper, wallet_client, wallet_client2):
    # Create signed request and convert to legacy txn
    req_str = json.dumps(vdr_random_request_objects(1, CURRENT_PROTOCOL_VERSION, wallet_client[1])[0].as_dict)
    req_str = vdr_multisign_request_object(looper, wallet_client, req_str)
    req_str = vdr_multisign_request_object(looper, wallet_client2, req_str)
    req = deserialize_req(req_str)
    txn = req_to_legacy_txn(req_str)
