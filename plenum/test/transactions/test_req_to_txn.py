...
def req_and_expected(request, looper, wallet_client, endorser):
    ...
    if request.param.endswith('_sdk'):
        ...
        if request.param == 'sigs_only':
            req = vdr_multisign_request_from_dict(looper, wallet_client,
                                                  op, reqId=1513945121191691,
                                                  taa_acceptance=taaa,
                                                  endorser=endorser)
        else:
            req = vdr_sign_request_from_dict(looper, wallet_client,
                                             op, reqId=1513945121191691,
                                             taa_acceptance=taaa,
                                             endorser=endorser)
