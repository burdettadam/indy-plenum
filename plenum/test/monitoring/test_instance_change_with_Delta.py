...
def step1(looper, txnPoolNodeSet, pool_handle, wallet_client):
...
def testInstChangeWithLowerRatioThanDelta(looper, step3, pool_handle, wallet_client):
    # from plenum.test.test_node import ensureElectionsDone
    # ensureElectionsDone(looper, [])

    vdr_send_random_and_check(looper, step3.nodes, pool_handle, wallet_client, 9)
    # wait for every node to run another checkPerformance
    waitForNextPerfCheck(looper, step3.nodes, step3.perfChecks)
    provoke_and_wait_for_view_change(looper, step3.nodes, 1, pool_handle, wallet_client)
