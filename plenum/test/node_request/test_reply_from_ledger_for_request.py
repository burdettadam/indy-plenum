...
def test_seq_no_db_signed_request(looper, node, wallet_client):
    # Create signed request and write it to ledger
    req = vdr_random_request_objects(1, identifier=wallet_client[1], protocol_version=CURRENT_PROTOCOL_VERSION)[0]
    req = vdr_sign_request_objects(looper, wallet_client, [req])[0]
...
def test_seq_no_db_multisigned_request(looper, node, wallet_client, wallet_client2):
    # Create signed request and write it to ledger
    req = vdr_random_request_objects(1, identifier=wallet_client[1], protocol_version=CURRENT_PROTOCOL_VERSION)[0]
    req = vdr_multisign_request_object(looper, wallet_client, json.dumps(req.as_dict))
    req = deserialize_req(req)
    write_request(node, req)
    ...
    multisig_req = vdr_multisign_request_object(looper, wallet_client2, json.dumps(req.as_dict))
...
def test_seq_no_db_unsigned_request(looper, node, wallet_client):
    ...
    signed_req = vdr_sign_request_objects(looper, wallet_client, [req])[0]
