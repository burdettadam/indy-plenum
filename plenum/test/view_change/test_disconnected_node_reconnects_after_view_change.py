...
def test_disconnected_node_with_lagged_view_pulls_up_its_view_on_reconnection(
        looper, txnPoolNodeSet, vdr_wallet_client, pool_handle, tconf):
    """
    ...
    vdr_send_batches_of_random_and_check(looper, txnPoolNodeSet, pool_handle, vdr_wallet_client, num_reqs=2 * tconf.CHK_FREQ)
    ensure_all_nodes_have_same_data(looper, txnPoolNodeSet)
