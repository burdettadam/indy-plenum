...
def sdk_send_freeze_ledgers(looper, pool_handle, sdk_wallets, ledgers_ids: List[int]):
...
def sdk_get_frozen_ledgers(looper, pool_handle, sdk_wallet):
    req = looper.loop.run_until_complete(ledger.build_get_frozen_ledgers_request(sdk_wallet[1]))
    rep = vdr_sign_and_submit_req_obj(looper, pool_handle, sdk_wallet, vdr_json_to_request_object(json.loads(req)))
